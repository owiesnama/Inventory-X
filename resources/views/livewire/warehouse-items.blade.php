<div>
            ...
            <div class="flex items-end justify-between mb-4">
                <div class="w-1/3 space-y-1">
                    <x-jet-input id="search" class="block w-full mt-1 " type="text" inputmode="numeric"
                        placeholder="Search ..." wire:model="search" name="search" />
                </div>
                <div class="space-y-1">
                        ...
                        <tr>
                            ...
                            <th scope="col" class="relative px-6 py-3 text-right">
                                <button class="px-6 py-2 text-white bg-indigo-600 rounded hover:bg-indigo-900"
                                    wire:click="toggleAddingModal">Add Stock</button>
                            </th>
                            ...
                            <tr :wire:key="'warehouse-'.$warehouse->id">
                                ...
                                <td class="px-6 py-4 space-x-4 text-sm font-medium text-right whitespace-nowrap">
                                    <a href="#" class="text-indigo-600 hover:text-indigo-900"
                                        wire:click="update({{ $stock->id }})">Edit</a> <a href="#"
                                        class="text-red-600 hover:text-red-900"
                                        wire:click="confirmingDeletion({{ $stock }})">Delete</a>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
                ...
                <div class="space-y-4">
                        ...
                        <div class="flex justify-center">
                            <div class="mb-3 xl:w-96">
                                ...
                                focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none"
                                    aria-label="Default select example" wire:model="stock.item_id">
                    ...
                    <div class="space-y-1">
                        ...
                        <x-jet-input id="address" class="block w-full mt-1" type="number" inputmode="numeric"
                            name="Address" x-ref="name" wire:model="stock.quantity"/>
                    </div>
                </div>
            </x-slot>
            <x-slot name="footer">
    ...
    <form method="post" wire:submit.prevent="destroy()">
        <x-jet-confirmation-modal wire:model="isDeleting">
            <x-slot name="title">
                Are you sure you want to delete <strong>{{ optional($stockToDelete)['name'] }}</strong>
            </x-slot>
            <x-slot name="content">
            </x-slot>
            <x-slot name="footer">
